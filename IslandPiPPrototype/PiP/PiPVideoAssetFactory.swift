...
enum PiPVideoAssetFactory {
    static func makeTinyBlackMovie() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("island-pip-surface.mp4")
        ...
        guard writer.canAdd(input) else {
            throw PiPVideoError.cannotAddInput
        }

        writer.add(input)

        guard writer.startWriting() else {
            throw writer.error ?? PiPVideoError.cannotStart
        }
        ...
        for second in 0...1 {
            guard input.isReadyForMoreMediaData,
                ...
                throw PiPVideoError.cannotAppend
            }
            ...
            guard status == kCVReturnSuccess else {
                throw PiPVideoError.bufferCreation
            }
        ...
        guard writer.status == .completed else {
            throw writer.error ?? PiPVideoError.cannotFinish
        }
