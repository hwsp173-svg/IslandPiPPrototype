...
final class PiPController: NSObject, ObservableObject {
    ...
    func configureIfNeeded() {
        ...
        guard AVPictureInPictureController.isPictureInPictureSupported() else {
            message = "Picture in Picture is unavailable on this device or configuration."
            configured = false
            return
        }
        ...
        do {
            ...
            guard let pip = AVPictureInPictureController(
                playerLayer: playerLayer
            ) else {
                configured = false
                message = "Unable to create the Picture in Picture controller."
                return
            }
            ...
            possibilityObservation = pip.observe(
...
extension PiPController: AVPictureInPictureControllerDelegate {
    ...
    nonisolated func pictureInPictureControllerDidStartPictureInPicture(
        ...
        Task { @MainActor in
            self.isActive = true
            self.message =
                "PiP active. The system owns size, position, and controls."
        }
    }
    ...
    nonisolated func pictureInPictureController(
        ...
        Task { @MainActor in
            self.isActive = false
            self.message =
                "PiP could not start: \(error.localizedDescription)"
        }
    }
...
struct PiPPlayerHost: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> PiPPlayerHostView {
        PiPPlayerHostView(
            layer: pip.playerLayer,
            onAppear: pip.hostDidAppear
        )
    }

    func updateUIView(
        _ uiView: PiPPlayerHostView,
        context: Context
    ) {
    }
}
